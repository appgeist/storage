...
const maybeTokenValidation = require('./maybeTokenValidation');

const URL_PATTERN = /^(\/(?:.+\/)?)(.{36})(?:-(\d{2,4})w)?(?:-(\d{2,4})h)?(-lq)?(\.\w+)$/;
...
module.exports = ({ storageDir, maxUrlCacheItems, maxAge, serveTokenValidationHandler }) => {
  const knownFiles = [];
  ...
  router.get(
    '*',
    // apply an authentication middleware if a function was provided
    ...maybeTokenValidation(serveTokenValidationHandler),
    async (req, res, next) => {
      try {
